...
bool is_bluff = false;      // Flag indicating a bluff. Regarded as false until proven otherwise.
...
void trigger_death_roulette(Player* player) {
    ...
    int chamber = rand() % chambers;           // Randomly pick a chamber

    if (chamber == 0) { // If it's the first chamber or guaranteed death
